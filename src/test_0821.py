...
from utils_0820 import get_dataset, kmeans_flat, exp_details, average_weights
...
if __name__ == '__main__':
    ...
    if args.load :
        ...
        clients_model, users_acc , users_loss = {}, {} , {}
        test_accuracy ,test_loss = [],[] #7/24 add
        all_test_acc , centers = 0.0 , 1# for first round
        dist = torch.tensor(0)
        epoch = 1 #7/24 add
        ...
        while args.test_acc > all_test_acc:
            ...
            for idx in selected_users:
                ...
                if idx not in clients_model:
                    ...
                    w, loss = local_model.update_weights(
                        model = copy.deepcopy(global_model), dist=dist ,global_round=epoch)
                    ...
                    local_model = LocalUpdate(args=args, dataset=train_dataset,
                                            idxs=user_groups[idx], logger=logger)
                    w, loss = local_model.update_weights(
                        model = clients_model[idx], dist=dist ,global_round=epoch) # assign global model to client then train)
                    local_weights.append(copy.deepcopy(w))
                    local_losses.append(copy.deepcopy(loss))
            ...
            for cluster in range(len(centers)):
                ...
                for idx in membership[cluster]:
                    ...
                    w = local_weights[i]
                    # lam: add global model weights as new local model weights (range: [0,2], default: 1.0)
                    w = OrderedDict({key: w[key]*0.5*(2-args.lam) + centers[labels[i]][key]*0.5*args.lam for key in w.keys()})
                    clients_model[idx].load_state_dict(w) # copy.deepcopy(w)
