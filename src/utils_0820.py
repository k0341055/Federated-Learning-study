...
def dbscan_cluster(w , clients_ndata, clients, args):
    ...
    for i, client_ndata in enumerate(clients_ndata):
        ...
        w_ls.append(w_flat)
        # for j in range(i + 1, num_vectors):
        #     dist_matrix[i, j] = torch.norm(w_ls[i] - w_ls[j]).to('cpu').numpy()
        #     dist_matrix[j, i] = dist_matrix[i, j]
    X = torch.stack(w_ls).to('cpu')
    dist_matrix = pairwise_distances(X.numpy(), metric='euclidean')
    # dc = round(np.percentile(dist_matrix[np.triu_indices(num_vectors, k=1)],args.dc_per),4)
    # 動態調整dc
    # mean_dist = np.mean(dist_matrix[np.triu_indices(num_vectors, k=1)])
    # std_dist = np.std(dist_matrix[np.triu_indices(num_vectors, k=1)])
    # dc = mean_dist + 0.5 * std_dist
    q1 = np.percentile(dist_matrix, 25)
    q3 = np.percentile(dist_matrix, 75)
    upper_bound = 2.5* q3 - 1.5 * q1 # q3 + 1.5 * (q3 - q1)
    # dc = np.median(np.median(np.where(dist_matrix != 0, dist_matrix, np.inf), axis=1))
    dc = max(np.min(np.where((dist_matrix <= 0) | (dist_matrix > upper_bound), np.inf, dist_matrix), axis=1))
    # dbscan
    min_samples = max(int(num_vectors/10),1)
    dbscan = DBSCAN(eps=dc, min_samples=min_samples, metric='precomputed')
    labels = dbscan.fit_predict(dist_matrix)
...
def dfs_cluster(w , clients_ndata, clients, args):
    ...
    # 後處理：合併小群和移除雜訊
    min_cluster_size =  max(int(num_vectors/10),1)  # 可以根據需要調整
    valid_clusters = []
    noise = []
...
def exp_details(args):
    ...
    if args.algo == 'fedavg':
        ...
        print(f'    Aggregation Algorithm : FedSEM')
    elif args.algo == 'fedsem_r':
        print(f'    Aggregation Algorithm : FedSEM_R')
    elif args.algo == 'feddbscan':
        print(f'    Aggregation Algorithm : FedDBSCAN')
    elif args.algo == 'feddfs':
        print(f'    Aggregation Algorithm : FedDFS')
