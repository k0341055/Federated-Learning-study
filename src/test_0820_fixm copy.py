...
if __name__ == '__main__':
    ...
    if args.load :
        ...
        while args.test_acc > all_test_acc: #0917
            ...
            for i,idx in enumerate(selected_users):
                ...
                if idx not in clients_model:
                    ...
                    clients_model[idx] = copy.deepcopy(global_model)
                    ...
                    local_model = LocalUpdate(args=args, dataset=train_dataset,
                                            idxs=user_groups[idx], logger=logger)
                    w, loss = local_model.update_weights(
                            model = clients_model[idx], dist = users_dist[idx] ,global_round=epoch) # dist # assign global model to client then train)
                    local_weights.append(copy.deepcopy(w))
                    local_losses.append(copy.deepcopy(loss))
            ...
            if -1 in membership:
                ...
                for idx in membership[-1]:
                    ...
                    w = OrderedDict({key: local_weights[i][key]*0.5*(2-args.lam) + w_avg[key]*0.5*args.lam for key in local_weights[i].keys()})
                    clients_model[idx].load_state_dict(w)
                    ...
                    label_acc , label_loss , test_acc, test_lo = local_test_inference(
                        args,clients_model[idx],test_dataset,users_label[idx],clients_ndata[i]*0.2)
            ...
            for cluster in range(len(centers)):
                ...
                for idx in membership[cluster]:
                    ...
                    if args.algo == 'fedsem':
                        ...
                        w = OrderedDict({key: local_weights[i][key]*0.5*(2-args.lam) + centers[cluster][key]*0.5*args.lam for key in local_weights[i].keys()})
                    clients_model[idx].load_state_dict(w) # w #local_weights[i]
                    local_model = LocalUpdate(args=args, dataset=train_dataset,
                                            idxs=user_groups[idx], logger=logger)
                    acc, loss = local_model.inference(model=clients_model[idx])
                    list_acc.append(acc * clients_ndata[i]) # weighted by train dataset in user
        ...
        if args.algo == 'fedsem':
            f.write(f'\n    Aggregation Algorithm : KMeans(FedSEM)')
        elif args.algo in 'kmeans':
            f.write(f'\n    Aggregation Algorithm : KMeans')
        elif args.algo in 'dbscan':
            f.write(f'\n    Aggregation Algorithm : DBSCAN')
        elif args.algo in 'dfs':
            f.write(f'\n    Aggregation Algorithm : DFS')
        ...
        if args.algo == 'fedsem':
            ...
            open('../save/json/{}/{}/test/{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}].json'.
            format(args.dataset, args.algo, args.algo, epoch, args.frac,  len(membership.keys()),
                    args.iid, args.unequal, args.one ,args.local_ep, args.local_bs),'w'))
        else:
            json.dump({'test_accuracy':test_accuracy,'test_loss':test_loss},\
            open('../save/json/{}/{}/{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}].json'.
            format(args.dataset, args.algo, args.algo, epoch, args.frac,  len(membership.keys()),
                    args.iid, args.unequal, args.one ,args.local_ep, args.local_bs),'w'))
    ...
    # j = json.load(open('../save/json/{}/{}/{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}].json'.
    #         format(args.dataset, args.algo, args.algo, epoch, args.frac,  len(membership.keys()),
    ...
    # plt.plot(range(len(j['test_loss'])), j['test_loss'], color='r',ls=':',label = '{} loss:{}_{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}]'.
    #             format(args.algo,args.dataset, args.model, epoch, args.frac,len(membership.keys()),
    #                    args.iid, args.unequal, args.one ,args.local_ep, args.local_bs))
    # plt.ylabel('Testing loss')
    # plt.xlabel('Communication Rounds')
    # plt.savefig('../save/img/{}_{}_{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}]_loss.png'.
    #             format(args.algo,args.dataset, args.model, epoch, args.frac, len(membership.keys()),
    ...
    # plt.plot(range(len(j['test_accuracy'])), j['test_accuracy'], c='b',ls=':',label = '{} acc:{}_{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}]'.
    #             format(args.algo,args.dataset, args.model, epoch, args.frac, len(membership.keys()),
    #                    args.iid, args.unequal, args.one ,args.local_ep, args.local_bs))
    # plt.legend(loc=0)
    ...
    # plt.xlabel('Communication Rounds')
    # plt.savefig('../save/img/{}_{}_{}_round[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_one[{}]_E[{}]_B[{}]_acc.png'.
    #             format(args.algo,args.dataset, args.model, epoch, args.frac, len(membership.keys()),
    #                    args.iid, args.unequal, args.one ,args.local_ep, args.local_bs)) # old: args.epochs