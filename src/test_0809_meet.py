...
if __name__ == '__main__':
    ...
    if args.load :
        ...
        while args.test_acc > max(test_acc_ls):
            selected_users = np.random.choice(idxs_users, m, replace=False)
            print('selected users:',selected_users)
            acc_avg_ls , list_loss_ls = [] , []# each cluster's train accuracy in a round
            ...
            for cluster in range(len(centers)):
                ...
                cluster_acc = sum(list_acc)/len(list_acc)
                cluster_loss = np.mean(np.array(list_loss))
                acc_avg_ls.append(cluster_acc)
                list_loss_ls.append(cluster_loss) # train loss in cluster
            ...
            #每round先取num_clusters中，各指標的平均值作為該round的結果
            train_loss.append(np.mean(list_loss_ls))
            train_accuracy.append(np.mean(acc_avg_ls))
            test_accuracy.append(np.mean(test_acc_ls))
            test_loss.append(np.mean(test_lo_ls))
            print(f' \nAvg Training Stats after {epoch} global rounds from multi-center:')
        ...
        f.write(f"\n|---- Test Loss: {100*test_loss[-1]}")
        f.write('\n Total Run Time: {0:0.4f}\n'.format(time.time()-start_time))
        f.close()
        for i,gm in enumerate(gm_ls):
            torch.save(gm, './save_model/multi-center[{}]_model_{}_{}_{}_C[{}]_iid[{}]_unequal[{}]_E[{}]_B[{}].pt'.\
            format(i,args.dataset, args.model, epoch, args.frac, args.iid, args.unequal,
        ...
        json.dump({'test_accuracy':test_accuracy,'test_loss':test_loss},\
            open('../save/json/multi-center_test_result_{}_{}_round[{}]_gpu[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_E[{}]_B[{}].json'.
            format(args.dataset, args.model, epoch, args.gpu, args.frac,  args.num_clusters,
    ...
    j = json.load(open('../save/json/multi-center_test_result_{}_{}_round[{}]_gpu[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_E[{}]_B[{}].json'.
            format(args.dataset, args.model, epoch, args.gpu, args.frac, args.num_clusters,
    ...
    plt.savefig('../save/img/multi-center_{}_{}_round[{}]_gpu[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_E[{}]_B[{}]_loss.png'.
                format(args.dataset, args.model, epoch, args.gpu, args.frac, args.num_clusters,
    ...
    plt.plot(range(len(j['test_accuracy'])), j['test_accuracy'], c='b',ls=':',label = 'multi-center FL test accuracy')
    plt.legend(loc=0)
    plt.axhline(y=args.test_acc, c='gray',ls='--')
    plt.ylabel('Testing Accuracy')
    plt.xlabel('Communication Rounds')
    plt.savefig('../save/img/multi-center_{}_{}_round[{}]_gpu[{}]_C[{}]_Clusters[{}]_iid[{}]_unequal[{}]_E[{}]_B[{}]_acc.png'.
                format(args.dataset, args.model, epoch, args.gpu, args.frac, args.num_clusters,
                       args.iid, args.unequal,args.local_ep, args.local_bs)) # old: args.epochs