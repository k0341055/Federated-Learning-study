...
if __name__ == '__main__':
    ...
    if args.load :
        ...
        all_test_acc , centers , dist = 0.0 , 1 , torch.tensor(0) # for first round
        clients_ndata, clients_model = [], {} 
        epoch = 1 #7/24 add
        ...
        while args.test_acc > all_test_acc:
            ...
            for idx in selected_users:
                ...
                if idx not in clients_model: # initialize local model with global model
                    ...
                    local_model = LocalUpdate(args=args, dataset=train_dataset,
                                            idxs=user_groups[idx], logger=logger)
                    w, loss = local_model.update_weights(
                        model = clients_model[idx], dist = users_dist[idx], global_round=epoch) # assign center^k to client then train)
            ...
            for cluster in range(len(centers)):
                ...
                for idx in membership[cluster]:
                    ...
                    # w = OrderedDict({key: w[key] + centers[labels[i]][key]*args.lam for key in w.keys()})
                    clients_model[idx].load_state_dict(centers[labels[i]]) #initialize local model with center^k # clients_model[idx].load_state_dict(local_weights[i])
